...
function createJSONGraph(m, n, d, randPercentage) {
    let graph = {};
    ...
    // random sequence as obstacles
    let obstacles = [];
    while (obstacles.length < entriesQuant * randPercentage) {
        let rand = Math.floor(Math.random() * entriesQuant) + 1;
        if (obstacles.indexOf(rand) == -1) obstacles.push(rand.toString());
    }
    ...
    for (let node = 1; node <= entriesQuant; ++node) {
        ...
        if (obstacles.indexOf(node.toString()) == -1) {
            /*  Matrix Cases:
            ...
            */
            ...
            // prepare graph key to receive connections
                    ...
                    // D
                        ...
                        || command[0] == "HERNAN") {
                            if (command[1] == "SI") {
                                ...
                                voiceProcessor.readOutLoud("Asignación inválida.");
                            }
                    } 
                    // CREATE
                        ...
                        if (m && n && a && d) {
                            let network = createJSONGraph(m, n, d, 0);
                            console.log("Network:");
